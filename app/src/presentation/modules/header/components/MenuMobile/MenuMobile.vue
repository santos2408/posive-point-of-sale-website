...
<template>
  <div class="menu-overlay fixed top-0 left-0 z-50 w-full">
    <nav ref="menu" class="bg-brand-neutral-500 z-50 h-screen w-4/5 px-4 py-8">
      <div class="mb-10 flex items-center justify-between">
        ...
        <button type="button" aria-label="Close menu" @click="() => emit('close-menu')">
          <X class="cursor-pointer" color="#fff" />
        </button>
      </div>
      ...
      <ul class="mb-5 text-sm font-medium text-white">
        <li
          ...
          :key="idx"
          :class="['border-t', 'py-1', 'border-white/10', 'border-opacity-10', idx === props.items.length - 1 ? 'border-b' : undefined]"
        >
          <NuxtLink :to="menuItem.link" class="block cursor-pointer py-3">{{ menuItem.label }}</NuxtLink>
        </li>
      </ul>
      ...
      <div class="flex flex-col gap-4">
        <div class="flex items-center gap-2 text-sm text-white">
          <div class="bg-brand-blue-100 flex h-7 w-7 items-center justify-center rounded-full">
            <LucideMail class="mt-0.5 size-5 text-white" />
          </div>
        ...
        <div class="mb-3 flex items-center gap-2 text-sm text-white">
          <span class="bg-brand-blue-100 flex h-7 w-7 items-center justify-center rounded-full">
            <LucidePhone class="mt-0.5 size-5 text-white" />
          </span>
